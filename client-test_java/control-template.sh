...
Start() {
    java $java_opts -jar ${filePah} -Dfile.encoding=utf-8 --spring.profiles.active=${envi} ${application_properties} -Djava.security.egd=file:/dev/./urandom --name=${module_name} &>/dev/null
}
Stop() {
    pid=$(ps -ef | grep -n "java.*--name=${module_name}" | grep -v grep | grep -v kill | awk '{print $2}')
    if [ ${pid} ]; then
        kill ${tpid}
        for ((i = 0; i < 10; ++i)); do
            sleep 1
