...
public class HBaseUtils {
    ...
    public Put buildPut(String row, String columnFamilies, String[] columns, Object[] values) {
        Put put = new Put(Bytes.toBytes(row));
        // 设置不写hlog
        put.setDurability(Durability.SKIP_WAL);
        for (int i = 0; i < columns.length; i++) {
            if (values[i] != null) {
                put.addColumn(Bytes.toBytes(columnFamilies), Bytes.toBytes(columns[i]), values[i] instanceof byte[] ? (byte[]) values[i] : Bytes.toBytes(values[i].toString()));
            } else {
                put.addColumn(Bytes.toBytes(columnFamilies), Bytes.toBytes(columns[i]), Bytes.toBytes("-"));
            }
        }
        return put;
    }
     ...
     * @return
     */
    public Put buildPut(String row, String columnFamilies, String[] columns, Object[] values, long timestamp) {
     ...
     * @throws Exception
     */
    public List<Map<String, Object>> fuzzyQuery(String tableName, String startRow, String stopRow, List<String> columns) throws Exception {
        ...
        if (columns.size() > 0) {
            columns.forEach(column -> scan.addColumn("cf".getBytes(), column.getBytes()));
        }
